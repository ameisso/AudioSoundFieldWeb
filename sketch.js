...
function preload() {
    ...
    player = new Sprite(150, 60, 20, 20, 'default');
    player.addAnimation('assets/player.png');

    bg = new Group();
    for (var i = 0; i < 80; i++) {
        var rock = new Sprite(random(-width, SCENE_W + width), random(-height, SCENE_H + height), 'static');
...
function draw() {
    ...
    // player.velocity.x = (camera.mouse.x - player.position.x) / 59;
    // player.velocity.y = (camera.mouse.y - player.position.y) / 59;
